...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec2 origuv = uv;
    uv.x = pow(abs(uv.x - 0.5),1.5) + 0.5;
    uv.y = abs(0.5 - uv.y);
    uv.y = pow(1.0 - pow(uv.y,0.5),0.5);

    number low = min(tex.r, min(tex.g, tex.b));
    number high = max(tex.r, max(tex.g, tex.b));
    number delta = high-low -0.1;

    number fac = 0.8 + 0.9*sin(11.*uv.x+4.32*uv.y + gilded.r*12. + cos(gilded.r*5.3 + uv.y*4.2 - uv.x*4.));
    number fac2 = 0.5 + 0.5*sin(8.*uv.x+2.32*uv.y + gilded.r*5. - cos(gilded.r*2.3 + uv.x*8.2));
    number fac3 = 0.5 + 0.5*sin(10.*uv.x+5.32*uv.y + gilded.r*6.111 + sin(gilded.r*5.3 + uv.y*3.2));
    number fac4 = 0.5 + 0.5*sin(3.*uv.x+2.32*uv.y + gilded.r*8.111 + sin(gilded.r*1.3 + uv.y*11.2));
    number fac5 = sin(0.9*16.*uv.x+5.32*uv.y + gilded.r*12. + cos(gilded.r*5.3 + uv.y*4.2 - uv.x*4.));
    ...
    vec4 orig = tex;
    number gG = (0.8 - delta) + (delta*maxfac*(0.7 - fac3*0.27));
    number rG = (0.8 - (0.1 * (gG - 0.8)));
    tex.r = (tex.r*rG*0.625 + (0.05*gilded.y));
    tex.g = (tex.g*gG + (0.02*gilded.x));
    tex.b = (tex.b * 0.325) + (tex.b * fac5 * 0.01);

    //number tot = orig.r + orig.g;
    //tex.g = tex.g * (sB * (orig.g / tot))
    //tex.g = tex.g * (1 + (orig.b / tot));
    //tex.r = tex.r * (1 + (orig.b / tot));

    // required
    return dissolve_mask(tex*colour, texture_coords, origuv);
}
