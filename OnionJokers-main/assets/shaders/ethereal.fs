...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float offset = sin((ethereal.y * 1.5) + (uv.x * 6.0) ) * 0.075;
    float edge_fader_y = 1.0 - (abs(uv.y - 0.5) * 2.0);
    float edge_fader_x = 1.0 - (abs(uv.x - 0.5) * 2.0);
    edge_fader_x = (edge_fader_x * 0.5) + 0.5;
    float edge_fader = pow(edge_fader_x,0.75) + edge_fader_y;
    //1.0 - (abs(uv.y - 0.5) + 0.5);

    offset *= 1.0 - (abs(uv.y - 0.5) + 0.5);
    texture_coords.y += offset;

    // Take pixel color (rgba) from `texture` at `texture_coords`, equivalent of texture2D in GLSL
    vec4 tex = Texel(texture, texture_coords);
    
    tex.r += 0.01 * sin(ethereal.x);
    tex.g -= 0.01 * sin(ethereal.x);
    tex.a *= ((tex.r + tex.g + tex.b) / 3) * max(edge_fader * 0.625,0.2);
    colour.rgb *= edge_fader + 0.125;
